...
class TestConversionCurve3D:
    ...
    def test_no_small_spurious_conversions(self):
        """Curve points should not have tiny amounts from grid interpolation.

        Regression test for GitHub issue #59: the 3D DP bilinear policy
        interpolation was producing small spurious amounts (e.g. $100-$200)
        in years that shouldn't have conversions at a given budget cap.
        """
        from app.engine.dp import MIN_CONVERSION

        # Use a scenario with mixed high/low income years where the
        # optimizer concentrates conversions in low-income years.
        ...
        curve = extract_conversion_curve_3d(scenario, n_points=50, balance_grid_size=300)

        for point in curve:
            for i, c in enumerate(point.yearly_conversions):
                assert c == 0 or c >= MIN_CONVERSION, (
                    f"At cap=${point.total_cap:,.0f}, year {2026+i}: "
                    f"conversion=${c:,.0f} is below MIN_CONVERSION=${MIN_CONVERSION}"
                )
