...
def federal_tax_on_conversion(
    ...
    return (
        calculate_federal_tax(base_income + conversion, filing_status)
        - calculate_federal_tax(base_income, filing_status)
    )
...
def state_tax_on_conversion(
    ...
    state: Optional[str],
    filing_status: FilingStatus,
    custom_state_rate: Optional[float] = None,
) -> float:
    """State tax cost of a conversion. Returns 0 if no state is provided."""
    if not state:
        return 0.0
    return (
        calculate_state_tax(base_income + conversion, state, filing_status, custom_state_rate)
...
def total_conversion_cost(
    ...
    cost += state_tax_on_conversion(base_income, conversion, state, filing_status, custom_state_rate)

    if healthcare and is_aca_year and conversion > 0:
        cost += calculate_subsidy_loss(
            base_income, conversion,
            healthcare.household_size, healthcare.monthly_slcsp_premium,
        )
